    ...
    foreach ($csv_template_data as $entry) {
        ...
        foreach ($csv_data as $row) {
            if (!empty($row[1]) && $slug === $row[1]) {
                ...
                if (!$is_child_page) {
                    $child_pages = vpg_get_child_pages($csv_data, $slug);
                } else {
                    $child_pages = [];
                }
                ...
                $values = array_merge([
                    ...
                    isset($row[48]) ? esc_html($row[48]) : ''], vpg_generate_child_page_values($child_pages)
                );
...
// function vpg_get_child_pages($csv_data, $parent_slug) {
//     $child_pages = array_filter($csv_data, function ($row) use ($parent_slug) {
//         return isset($row[48]) && $row[48] === $parent_slug;
//     });

//     $child_pages = array_values($child_pages);

//     if (count($child_pages) > 3) {
//         $random_keys = array_rand($child_pages, 3);
//         $selected_pages = [];
//         foreach ((array) $random_keys as $key) {
//             $selected_pages[] = $child_pages[$key];
//         }
...
// function vpg_get_child_pages($csv_data, $parent_slug) {
//     error_log("Checking for children of parent: " . $parent_slug);
    
//     foreach ($csv_data as $row) {
//         error_log("Row Parent Slug: " . isset($row[48]) ? $row[48] : 'EMPTY');
//     }

//     $child_pages = array_filter($csv_data, function ($row) use ($parent_slug) {
//         return isset($row[48]) && trim($row[48]) === trim($parent_slug);
//     });

//     error_log("Total Child Pages Found After Filtering: " . count($child_pages));

//     foreach ($child_pages as $child) {
//         error_log("Child Slug: " . $child[1]);
//     }

//     return array_values($child_pages);
// }
...
//     foreach ($csv_data as $row) {
//         $row_parent_slug = isset($row[48]) ? trim($row[48]) : 'EMPTY';
//         error_log("Row Parent Slug: " . $row_parent_slug . " | Expected: " . $parent_slug);

//         if (!empty($row_parent_slug) && $row_parent_slug === trim($parent_slug)) {
//             $child_pages[] = [
//                 'title'    => isset($row[4]) ? $row[4] : '',
//                 'hero_img' => isset($row[7]) ? $row[7] : '',
//                 'url'      => home_url($row[1])
...
//     foreach ($child_pages as $child) {
//         error_log("Child Slug: " . $child['url']);
//     }
...
function vpg_get_child_pages($csv_data, $current_slug) {
    $child_pages = [];

    foreach ($csv_data as $row) {
        // Ensure the row has at least 49 columns (to avoid undefined index errors)
        if (count($row) < 49) {
            continue;
        }

        // Get the Parent column value (name of the parent category)
        $row_parent_name = isset($row[48]) ? trim($row[48]) : 'EMPTY';

        // Convert Parent Name to Parent Slug
        $row_parent_slug = ($row_parent_name !== 'EMPTY') ? vpg_get_parent_slug($csv_data, $row_parent_name) : '';

        // Debugging output (remove in production)
        error_log("Checking Row: Slug = {$row[1]}, Parent Name = {$row_parent_name}, Converted Parent Slug = {$row_parent_slug}");

        // Check if the converted Parent Slug matches the current page slug
        if ($row_parent_slug === $current_slug) {
            $child_pages[] = [
                'title' => isset($row[0]) ? trim($row[0]) : '',
                'slug'  => isset($row[1]) ? trim($row[1]) : '',
                'hero_img' => isset($row[7]) ? trim($row[7]) : '', // Use 'hero_img' instead of 'hero_img_url'
                'url' => home_url($row[1]) // Generate URL
            ];
        }
    }
    ...
    foreach ($csv_template_data as $entry) {
        ...
        foreach ($csv_data as $row) {
            if (!empty($row[1]) && $slug === $row[1]) {
                ...
                $placeholders = ['[vpg_hero_heading]', '[vpg_hero_subheading]', '[vpg_hero_cta]', '[vpg_hero_img_url]',
                    '[vpg_challenge_title]',
                    '[vpg_challenge_1_title]', '[vpg_challenge_1_des]', '[vpg_challenge_2_title]', '[vpg_challenge_2_des]', '[vpg_challenge_3_title]', '[vpg_challenge_3_des]',
                    '[vpg_usecase_heading]',
                    '[vpg_usecase_1_heading]', '[vpg_usecase_1_content]', '[vpg_usecase_1_img_url]',
